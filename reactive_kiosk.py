        ...
        if len(faces) == 0:
            ...
            for (x, y, w, h) in faces:
                ...
                if len(eye_centers) == 2:
                    ...
                    if stable_position and (time.time() - start_time >= 15):
                        # 얼굴 인식 완료, 모터 제어 멈춤
                        arduino.write(b'STOP\n')
                        cap.release()
...
def recognize_speech():
    ...
    with mic as source:
        ...
        recognizer.adjust_for_ambient_noise(source, duration=1)  # 환경 소음 조정 시간을 늘림
        audio = recognizer.listen(source, timeout=5)  # 마이크로부터 음성 입력 시간을 늘림

    try:
        speech = recognizer.recognize_google(audio, language="ko-KR", show_all=False)
        print(f"사용자: {speech}")
        return speech
...
# JPG 파일을 표시하는 함수
def display_image(image_path, window_name="Image", delay=3000):
...
def main():
    while True:
        if detect_and_move():
            ...
            speak("어서오세요 원광 카페입니다. 주문을 도와드릴게요.")
            
            speak("간편 주문화면 으로 바꾸시겠습니까?")
            easy_ui = input("쉬운 UI로 바꾸시겠습니까? (Y/N): ").strip().upper()
            ...
            if easy_ui == 'Y':
                ...
                speak("간편 주문 화면으로 변경되었습니다. 음성으로 주문하시겠습니까? ")
